...
class VisualSimilarityEngine:
    ...
    def compute_all_pairwise_similarities(self, db: Session) -> int:
        ...
        valid_items = [
            ...
            if os.path.isfile(f.path) and (not a or (not a.is_screenshot and a.analysis_status != "skipped_text_heavy" and a.visual_category not in ["scanned_document", "document", "text_heavy"]))
        ]
    ...
    def generate_visual_groups(self, db: Session) -> int:
        ...
        analyses = db.query(MediaAnalysis, File).join(File, MediaAnalysis.file_id == File.id).filter(
            ...
            MediaAnalysis.is_screenshot == False,
            File.extension.in_(list(VALID_MEDIA_EXTENSIONS))
        ).all()
        ...
        pictorial_analyses = [
            ...
            if a.analysis_status != "skipped_text_heavy" and a.visual_category not in ["scanned_document", "document", "text_heavy"]
        ]
        ...
        for a, f in pictorial_analyses:
            objs = a.get_detected_objects() if a else []
            scenes = a.get_detected_scenes() if a else []
            items_payload.append({
                "file_id": f.id,
                "filename": f.name,
                "description": (a.visual_category if a else "") or f.name,
                "objects": objs,
                "scenes": scenes,
                "tags": objs + scenes
            })
