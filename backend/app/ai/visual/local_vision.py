        ...
        try:
            ...
            if client:
                try:
                    ...
                    logger.warning(f"[LOCAL VISION] Ollama client chat failed: {client_e}")

            if not raw_text:
                error_msg = f"Ollama visual analysis error: {http_e}" if 'http_e' in locals() else "Ollama visual analysis failed to return visual metadata."
                logger.warning(f"[LOCAL VISION] Ollama visual analysis fallback for {filename}: {error_msg}")
                return {
                    "status": "failed",
                    "content_type": "pictorial",
                    "classification_confidence": 0.8,
                    "classification_reason": "Ollama visual analysis unavailable",
                    "description": "",
                    "error": error_msg,
                    "objects": [],
                    "object_counts": [],
                    "scene": "photograph",
                    "detected_scenes": ["photograph"],
                    "environment": "general",
                    "activities": [],
                    "visual_attributes": [],
                    "relationships": [],
                    ...
                    "searchable_text": f"Visual photograph {filename}"
                }
